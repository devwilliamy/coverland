...
const CoversGrid = () => {
  return (
    <span className="mt-[-70px] h-full flex-col items-center lg:flex lg:px-[80px]">
      <div className="grid h-full max-w-[1280px]  grid-cols-2 grid-rows-2 place-items-center gap-[7px] lg:gap-[20px]  ">
        {coverTypes.map(({ title, img }) => (
          <div
            key={`${title}-block`}
            className="flex min-w-[167px] max-w-[167px] flex-col items-center justify-center rounded-[8px] p-[10px] shadow-md lg:flex-row lg:gap-[36px] lg:px-[50px]"
          >
            <p className="order-last flex min-w-[45%] flex-[0.1] justify-center whitespace-nowrap text-center align-middle text-[14px] font-[900] uppercase lg:w-1/2 lg:max-w-[45%] lg:text-[24px]">
              {title}
            </p>
