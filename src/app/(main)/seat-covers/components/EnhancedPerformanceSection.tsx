...
export default function EnhancedPerformanceSection() {
  ...
  const fullSeat =
    isFullSet(selectedProduct?.display_set) === 'full' ? true : false;
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center">
      ...
      <section className={sectionStyle}>
        {enhancedPerformanceData.map(
          ({ img, alternativeImg, title, id }, index) =>
            excludedIds.includes(id) && !fullSeat ? null : (
              <div
                key={index}
                className="flex h-[360px] w-full flex-col items-center justify-center "
              >
                <Image
                ...
                />
                <p className="w-full bg-[#717171] py-[10px] text-center text-white">
                  {title}
                </p>
              </div>
            )
